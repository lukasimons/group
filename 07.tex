...
\begin{proof}
    ...
    We now prove that $\sigma\circ\tau=\id_\mathcal{B}$, this is, if  $Y\in\mathcal{B}$ then $\sigma(\tau(Y))=Y$. If $y\in Y\subseteq f(G)$, then $y=f(x)$ for some $x\in G$, that is $x\in f^{-1}(y)$. This implies that $y=f(x)\in f(f^{-1}(Y))$. Conversely, if $y\in f(f^{-1}(Y))$, then $y=f(x)$ for some $x\in f^{-1}(Y)$. This implies that $y=f(x)\in Y$.
...
\begin{corollary}
    If $f\colon G\to H$ is a group homomorphism and $L\leq G$ such that $K = \ker f \subseteq L$, then $(G\colon L) = (f(G)\colon f(L)$.
\end{corollary}
\begin{proof}
Consider the set $\{gLg^{-1}\mid g\in G}$. By normality of the kernel, this is a subset of $\{L'\mid K\leq L'\leq G\}$, and by the Correspondence Theorem this is in bijection with the set $\{f(g)f(H)f(g)^{-1}\mid g\in G\}$. Hence both sets are of equal size, proving the claim.
\end{proof}
...
subgroups correspond with three non-normal subgroups of $G$, all of order 15. 
\end{example}
...
we see that every finite group is 
